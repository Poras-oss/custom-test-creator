...
const ControlButton = ({ active, children, isDarkTheme, ...props }: { active: boolean; children: React.ReactNode; isDarkTheme: boolean } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
  <button
    {...props}
    className={`flex-1 px-4 py-2.5 rounded-lg text-base font-semibold transition-all duration-300 ${active
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Failed to track custom test:", trackErr);
        // We don't block the user if tracking fails
      }
                ...
                <div className="flex flex-wrap gap-2 overflow-y-auto">
                  {currentSubtopics.map(sub => (
                    <button key={sub} onClick={() => handleSubtopicClick(sub)}
                      className={`px-4 py-2 rounded-md text-sm font-medium transition-colors border ${formData.subtopics.includes(sub)
